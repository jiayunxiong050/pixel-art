...
export interface MardColor {
  ...
  brandIds?: Record<Brand, string>;
}

// 品牌色号映射数据 (完整 292 色系)
const BRAND_MAPPINGS: Record<string, Record<Brand, string>> = {
...
function hexToColorName(hex: string): string {
  ...
  // 根据色相判断主色调
  let hue = "";
  if (h < 0.05 || h >= 0.92) hue = "红";
  else if (h < 0.17) hue = "橙";
  else if (h < 0.28) hue = "黄";
  else if (h < 0.42) hue = "绿";
  else if (h < 0.58) hue = "青";
  else if (h < 0.67) hue = "蓝";
  else if (h < 0.78) hue = "紫";
  else hue = "粉";

  // 根据明度和饱和度加修饰
  if (l < 0.2) return `深${hue}`;
  if (l > 0.85 && s < 0.2) return "白色";
  if (l < 0.35) return `深${hue}`;
  if (l > 0.7 && s < 0.3) return "浅灰";
  if (s < 0.15) return "灰色";
  if (l > 0.6 && s > 0.5) return `浅${hue}`;
  if (s > 0.8) return `${hue}色`;

  return `${hue}色`;
}

/**
 * 从 BRAND_MAPPINGS 键名解析 RGB
 */
...
// 从 BRAND_MAPPINGS 构建完整的 292 色列表（MARD 色号为 id）
export const MARD_COLORS: MardColor[] = Object.entries(BRAND_MAPPINGS).map(([hex, brandIds]) => {
  const mardId = brandIds["MARD"];
  return {
    id: mardId,                     // 使用 MARD 色号作为唯一 id
...
// 按 MARD 色号排序（A01 → ZG8）
export const MARD_COLORS_SORTED = [...MARD_COLORS].sort((a, b) => a.id.localeCompare(b.id));
...
// MARD_COLORS_LAB 在 rgbToLab 函数定义之后定义（见上方）
...
export async function pixelateImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (ratio > 1) {
        height = Math.round(gridSize / ratio);
      } else {
        width = Math.round(gridSize * ratio);
      }
      ...
      const grid: MardColor[][] = [];
      const usedColorsMap = new Map<string, MardColor>();
      ...
      for (let y = 0; y < height; y++) {
        ...
        for (let x = 0; x < width; x++) {
          ...
          if (a < 128) {
            ...
            row.push(closest);
            usedColorsMap.set(closest.id, closest);
          }
        }
        grid.push(row);
      }
...
/**
 * 将网格数据转换为一个用于 Fabric.js 的 Canvas 元素
 */
